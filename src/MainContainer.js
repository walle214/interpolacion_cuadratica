...
class Form extends Component {
  ...
  calcX = (_) => {
    ...
    const fx =
      (((x - valuesX[1]) * (x - valuesX[2])) /
        ((valuesX[0] - valuesX[1]) * (valuesX[0] - valuesX[2]))) *
        valuesFx[0] +
      (((x - valuesX[0]) * (x - valuesX[2])) /
        ((valuesX[1] - valuesX[0]) * (valuesX[1] - valuesX[2]))) *
        valuesFx[1] +
      (((x - valuesX[0]) * (x - valuesX[1])) /
        ((valuesX[2] - valuesX[0]) * (valuesX[2] - valuesX[1]))) *
        valuesFx[2];
  ...
  handleChanceX = (e) => this.setState({ textX: e.target.value });

  handleChanceFx = (e) => this.setState({ textFx: e.target.value });
          ...
          <div className='row mt-4'>
            <div className='col'>
              <label className='form-label'>
                Escribe X<sub>{this.state.currentX}</sub>
              </label>
              <input
                type='text'
            ...
            <div className='col'>
              <label className='form-label'>
                Escribe Fx<sub>{this.state.currentX}</sub>
              </label>
              <input
                type='text'
                className='form-control'
            ...
            <input
              value='Guardar'
              type='submit'
              className='form-control btn btn-outline-primary m-3'
            />
          </div>
        </form>
